...
public final class OptimizationLevelAllocator {
    ...
    public void normalizeSlot(BuildState state, SlotContext slot, OptimizationContext context) {
        ...
        for (Integer index : adjustableIndices) {
            ...
            state.setPlacement(
                    slot.key(), index, new Placement(current.drif(), selectedLevel, false));
        }
    }
    ...
    private List<Integer> resetUnlockedPlacementsToBaseLevel(
        ...
        for (int index = 0; index < placementLimit; index++) {
            ...
            int baseLevel = Math.min(BASE_TIER_MAX_LEVEL, placement.drif().getSize().getMaxLevel());
            state.setPlacement(
                    slot.key(), index, new Placement(placement.drif(), baseLevel, false));
