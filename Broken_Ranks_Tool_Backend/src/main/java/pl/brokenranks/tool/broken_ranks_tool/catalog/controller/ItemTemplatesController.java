...
public class ItemTemplatesController {
    ...
    public ResponseEntity<List<ItemTemplateDto>> getAllItems() {
        return ok(itemRepository.findAll().stream().map(ItemTemplateDto::fromEntity).toList());
    }
    ...
    public ResponseEntity<List<ItemTemplateDto>> getItemsByCategory(
            @PathVariable ITEM_CATEGORY category) {
        List<ItemTemplate> items = itemRepository.findByCategory(category);
        if (items.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        return ok(items.stream().map(ItemTemplateDto::fromEntity).toList());
    }
}
