...
public final class MaximizedDrifBonusPrelock {
    ...
    private void prelockType(
        ...
        int target =
                Math.min(
                        minQuantity(type, context.request()), maxQuantity(type, context.request()));
    ...
    private boolean tryPrelock(BuildState state, SlotContext slot, DRIF_BONUS_TYPE type) {
        ...
        state.setPlacement(
                slot.key(), position, new Placement(drif, drif.getSize().getMaxLevel(), true));
        return true;
    }

    private DrifTemplate maximumFittingDrif(
            SlotContext slot, List<Placement> placements, DRIF_BONUS_TYPE type) {
        ...
        return slot.candidates().stream()
                ...
                .filter(
                        candidate ->
                                power(candidate, candidate.getSize().getMaxLevel())
                                        <= remainingPower)
                .findFirst()
                .orElse(null);
    }
