...
const getRarityColor = (rarity) => {
    ...
    switch (rarity.toUpperCase()) {
        case "SET":
            return "bg-clip-text text-transparent bg-gradient-to-r from-green-400 to-green-600 font-bold";
        case "EPIC":
            return "bg-clip-text text-transparent bg-gradient-to-r from-yellow-400 to-yellow-600 font-bold";
        case "LEGENDARY":
            return "bg-clip-text text-transparent bg-gradient-to-r from-orange-400 to-orange-600 font-bold";
        case "RARE":
            return "bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-blue-600 font-bold";
        default:
            return "bg-clip-text text-transparent bg-gradient-to-r from-stone-400 to-stone-500 font-bold";
    }
};
        ...
        <div className="item-database-theme bg-gradient-to-b from-stone-900 to-black p-6 border-2 border-stone-800 shadow-[0_0_30px_rgba(0,0,0,0.9)] flex flex-col h-full relative">
            ...
            <div className="flex bg-black/60 p-1 mb-4 shrink-0 border border-stone-800 shadow-[inset_0_0_10px_rgba(0,0,0,1)]">
                {["items", "orbs", "drifs"].map((tab) => {
                    ...
                    const baseBtn =
                        "flex-1 py-2 text-xs font-serif font-bold uppercase tracking-widest transition-all";
                                    ...
                                    if (activeTab === "items") {
                                        return (
                                            <li
                                                ...
                                                draggable
                                                onDragStart={(e) =>
                                                    handleDragStart(e, itemData, "items")
                                                }
                                                onMouseMove={(e) =>
                                                    handleMouseMove(e, itemData, "items")
                                                }
                                            ...
                                            >
                                                <span
                                                    className={`truncate mr-2 font-serif ${getRarityColor(itemData.rarity)}`}
                                                >
                                                    {itemData.name ||
                                        ...
                                        const translatedName =
                                            bonusTranslations[baseItem.bonusType] ||
                                            baseItem.bonusType ||
                                            "";
                                        ...
                                        return (
                                            ...
                                            >
                                                <span
                                                    ...
                                                    onMouseMove={(e) =>
                                                        handleMouseMove(e, baseItem, activeTab)
                                                    }
                                                    onMouseLeave={handleMouseLeave}
                                                >
                                                    {baseItem.name && (
                                                        <span
                                                            className={`font-serif font-bold bg-clip-text text-transparent bg-gradient-to-r ${activeTab === "orbs" ? "from-red-400 to-rose-600" : "from-orange-400 to-amber-600"}`}
                                                        >
                                                            {baseItem.name}
                                                        </span>
                                                    )}
            ...
            {tooltip.show && tooltip.item && (
                ...
                >
                    <div className="flex justify-between items-start border-b-2 border-double border-rose-900/50 pb-2 mb-2">
                        <h4
                            className={`text-base font-serif tracking-wide ${tooltip.type === "items" ? getRarityColor(tooltip.item.rarity) : `font-bold bg-clip-text text-transparent bg-gradient-to-r ${tooltip.type === "orbs" ? "from-red-400 to-rose-600" : "from-orange-400 to-amber-600"}`}`}
                        >
                            {tooltip.type === "items" ? (
                                tooltip.item.name
                            ) : (
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-stone-400 text-xs font-normal">
                                        {bonusTranslations[tooltip.item.bonusType] ||
                                            tooltip.item.bonusType}
                                    </span>
                                </div>
                            )}
                        </h4>
                        {(tooltip.item.tier || tooltip.item.size) && (
                            <span
                                className={`text-xs font-serif font-bold mt-1 ml-2 ${tooltip.type === "orbs" ? "text-rose-800" : "text-orange-600"}`}
                            >
                                {tooltip.item.tier || tooltip.item.size}
                            </span>
                        )}
                    ...
                    {tooltip.type === "items" &&
                        (tooltip.item.stats && Object.keys(tooltip.item.stats).length > 0 ? (
                            Object.entries(tooltip.item.stats).map(([k, v]) => (
                                ...
                                >
                                    <span className="text-stone-400 font-serif uppercase tracking-wider">
                                        {k}
                                    </span>
                                    <span className="text-stone-300 font-bold font-serif">
                                        +{v}
                                    </span>
                                </div>
                            ))
                        ) : (
                            <p className="text-xs text-stone-600 font-serif italic mt-2">
                                Brak statystyk bazowych.
                            </p>
                        ))}
                        ...
                        <div className="flex flex-col gap-1.5 text-xs font-serif mt-2">
                            ...
                            <div className="flex justify-between mb-2 pb-1 border-b-2 border-double border-rose-900/30">
                                ...
                                <span className="text-orange-400 font-bold">
                                    {drifBasePowers[tooltip.item.bonusType] || "?"} pkt
                                </span>
                            </div>
                            ...
                            <div className="flex justify-between text-stone-400">
                                <span>Subdrif (Lvl 1-6):</span>{" "}
                                <span className="text-stone-300">x1</span>
                            </div>
                            <div className="flex justify-between text-stone-400">
                                <span>Bidrif (Lvl 7-11):</span>{" "}
                                <span className="text-stone-300">x2</span>
                            </div>
                            <div className="flex justify-between text-stone-400">
                                <span>Magnidrif (Lvl 12-16):</span>{" "}
                                <span className="text-stone-300">x3</span>
                            </div>
                            <div className="flex justify-between text-stone-400">
                                <span>Arcydrif (Lvl 17-21):</span>{" "}
                                <span className="text-orange-500">x4</span>
                            </div>
                        </div>
                    )}
